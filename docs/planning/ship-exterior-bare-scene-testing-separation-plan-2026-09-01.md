...
**Unresolved intermittency.** `ship-exterior-hangar-resume.spec.ts` "keeps scanned asteroid state after ship specs and View Exterior round-trip" failed once, then passed on re-run with no change to that spec or to production code, so it is intermittent rather than fixed. In the failing run the application state was provably correct: the genuinely Iron asteroid was scanned, the objective advanced, and `TARGET IRON` rendered. Only the "View Specs" click failed to move the right outlet, with no interception error and no competing navigation. No mechanism was confirmed.

Characterization: `--repeat-each=10` against that test in isolation passed 11/11, so the failure is not reproducible in isolation. The original failure occurred during a full-suite run, which is consistent with the load sensitivity already documented for this suite. An isolated repeat therefore cannot demonstrate either the presence or the absence of this flake.

**Mission upsert fixture contract defect.** All seven E2E `mission-upsert-request` handlers returned responses without `correlationId` or `requestIdentity`. `MissionService.upsertMissionStatus` discards any response failing that correlation check, so every mission synchronization settled through its full 5000 ms timeout rather than resolving. That window matches Playwright's default assertion timeout, and scan-completion synchronization made the dangling timer far more common. Fixed with a shared `missionUpsertCorrelationEcho` helper in `mission-session-helpers.ts` applied to all seven handlers. This is justified as a contract fix on its own merits; it is **not** established that it resolves the intermittency above, which was never reproduced under controlled conditions.

**Final result.** Unit, typecheck, build, and full E2E all pass on the merged branch, both before and after the fixture correlation fix. The hangar-resume intermittency should be watched across subsequent full runs rather than treated as closed.
