...
5. From the player perspective: sensor-array appears to succeed but the inventory shows drone instead.

Root cause layers:
1. item-upsert.ts: ItemUpsertResponse does not require correlationId or request identity echo.
2. socket.service.ts: listener accepts first matching event without correlation validation.
...
Clients must:
1. Before processing a response, validate that `response.correlationId === activeRequest.correlationId`.
2. Validate that `response.requestIdentity` matches the originating request identity.
...
- laughing-octo-journey/docs/planning/sw-08-contract-safety-gate-runbook.md
- solid-train/docs/planning/sw-08-contract-safety-gate-requirements.md
- solid-train/docs/planning/sw-08-cross-repo-index.md
