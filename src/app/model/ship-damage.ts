...
function coerceShipDamageSeverity(raw: unknown): ShipDamageSeverity | null {
  ...
  return (SHIP_DAMAGE_SEVERITIES as readonly string[]).includes(normalized) ? (normalized as ShipDamageSeverity) : null;
}
...
function coerceShipSubsystemDamage(raw: unknown): ShipSubsystemDamage | null {
  ...
  const repairPriority =
    typeof source['repairPriority'] === 'number' && Number.isInteger(source['repairPriority'])
...
export function coerceShipDamageProfile(raw: unknown): ShipDamageProfile | null {
  ...
  const systems = Array.isArray(source['systems'])
    ? source['systems']
        .map((entry) => coerceShipSubsystemDamage(entry))
        .filter((entry): entry is ShipSubsystemDamage => entry !== null)
    : [];
  const updatedAt =
