...
describe('printable-item', () => {
  ...
  describe('hasPrintableItemInInventory', () => {
    it('returns true when inventory contains the item type', () => {
      expect(hasPrintableItemInInventory([hullPatchKitItem], HULL_PATCH_KIT_PRINTABLE_ITEM)).toBe(true);
    });

    it('returns false when inventory does not contain the item type', () => {
      expect(hasPrintableItemInInventory([ironItem], HULL_PATCH_KIT_PRINTABLE_ITEM)).toBe(false);
    });

    it('returns false for empty inventory', () => {
      expect(hasPrintableItemInInventory([], HULL_PATCH_KIT_PRINTABLE_ITEM)).toBe(false);
    });

    it('returns false for undefined inventory', () => {
      expect(hasPrintableItemInInventory(undefined, HULL_PATCH_KIT_PRINTABLE_ITEM)).toBe(false);
    });
    ...
    it('returns false when matching item is destroyed', () => {
      ...
      expect(hasPrintableItemInInventory([destroyedKit], HULL_PATCH_KIT_PRINTABLE_ITEM)).toBe(false);
    });
  });

  describe('isPrintableItemQueued', () => {
    it('returns true when the item type is in the queue', () => {
      expect(isPrintableItemQueued([{ itemType: 'hull-patch-kit' }], HULL_PATCH_KIT_PRINTABLE_ITEM)).toBe(true);
    });

    it('returns false when the item type is not in the queue', () => {
      expect(isPrintableItemQueued([{ itemType: 'conduit-seals' }], HULL_PATCH_KIT_PRINTABLE_ITEM)).toBe(false);
    });

    it('returns false for empty queue', () => {
      expect(isPrintableItemQueued([], HULL_PATCH_KIT_PRINTABLE_ITEM)).toBe(false);
    });

    it('returns false for undefined queue', () => {
      expect(isPrintableItemQueued(undefined, HULL_PATCH_KIT_PRINTABLE_ITEM)).toBe(false);
    });
  });
