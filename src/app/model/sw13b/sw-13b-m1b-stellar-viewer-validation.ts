...
export interface Sw13bM1bFallbackNotes {
...
export interface Sw13bM1bStellarViewerValidationArtifacts {
  milestone: 'M1B';
  date: string;
  owner: string;
  contentReviewer: string;
  deterministicEvidence: Sw13bM1bDeterminismEvidence;
  readabilityNotes: Sw13bM1bReadabilityNotes;
  fallbackNotes: Sw13bM1bFallbackNotes;
...
function buildReadabilityNotes(samples: readonly AsteroidScanSample[]): Sw13bM1bReadabilityNotes {
  let baselineCount = 0;
  let heroCount = 0;
  ...
  return {
    ...
    notes: [
      baselineProfileReadable
        ? 'Baseline profile readability confirmed in Stellar Viewer generation sample set.'
        : 'Baseline profile readability not confirmed from generated sample set.',
      heroProfileReadable
        ? 'Hero profile readability confirmed in Stellar Viewer generation sample set.'
        : 'Hero profile readability not confirmed from generated sample set.',
      `Metadata completeness ${metadataCompleteCount}/${samples.length} for SW-13B debug/readability cues.`,
    ],
  };
}

function buildFallbackNotes(): Sw13bM1bFallbackNotes {
  ...
  const resumedBodies: CelestialBodyListItem[] = [
    {
      id: 'sv-probe-cb-1',
      catalogId: 'sv-probe-cat-1',
      sourceScanId: 'sample-a1',
      createdByCharacterId: 'm1b-probe-char',
  ...
  const resumedSamples = FIRST_TARGET_SHIP_EXTERIOR_MISSION.createResumedAsteroidSamples({
    playerName: 'm1b-probe-user',
    characterId: 'm1b-probe-char',
    center: { x: 1200, y: -300, z: 850 },
    launchSeedHint: 19,
...
function buildGapList(params: {
  ...
  if (!params.deterministicEvidence.allRunsIdentical) {
    gaps.push({
      gapId: 'GAP-SW13B-M1B-001',
      summary: 'Determinism mismatch detected for repeated Stellar Viewer seed/bundle runs.',
      status: 'open',
      owner: params.owner,
  ...
  if (!params.readabilityNotes.baselineProfileReadable || !params.readabilityNotes.heroProfileReadable) {
    gaps.push({
      gapId: 'GAP-SW13B-M1B-002',
      summary: 'Baseline/hero readability evidence is incomplete for Stellar Viewer M1B sample set.',
      status: 'open',
      owner: params.owner,
      contentReviewer: params.contentReviewer,
      targetClosureDate: params.targetClosureDate,
  ...
  if (!params.fallbackNotes.fallbackBehaviorConfirmed) {
    gaps.push({
      gapId: 'GAP-SW13B-M1B-003',
      summary: 'Fallback behavior evidence is incomplete for Stellar Viewer M1B sample set.',
      status: 'open',
      owner: params.owner,
      contentReviewer: params.contentReviewer,
      targetClosureDate: params.targetClosureDate,
...
export function buildSw13bM1bStellarViewerValidationArtifacts(params: {
  date: string;
  owner: string;
  contentReviewer: string;
  targetClosureDate: string;
  runCount?: number;
}): Sw13bM1bStellarViewerValidationArtifacts {
...
export const SW13B_M1B_STELLAR_VIEWER_VALIDATION: Sw13bM1bStellarViewerValidationArtifacts =
