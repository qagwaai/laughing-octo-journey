...
describe('SW-13B M0B asteroid baseline artifacts', () => {
  ...
  it('produces strict deterministic signatures across repeated and environment runs', () => {
    ...
    expect(env.stableAcrossEnvironments).toBeTrue();
  ...
  it('applies soft-warning performance gate and flags dense scenes', () => {
    ...
    expect(flagged.some((row) => row.sceneId === 'sv-m0b-dense-hero-mix-01')).toBeTrue();
  });
  ...
  it('confirms no marketplace or commissioned asteroid dependencies', () => {
    ...
    expect(artifacts.confirmations.noMarketplaceAsteroidDependencies).toBeTrue();
    expect(artifacts.confirmations.noCommissionedAsteroidDependencies).toBeTrue();
  });
});
