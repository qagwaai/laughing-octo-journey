...
describe('SW-13B visual foundation (Phase A)', () => {
  ...
  it('maintains hero-vs-baseline complexity separation over a 24-seed set', () => {
    ...
    expect(separation.passed).toBe(true);
  });
  ...
  it('builds deterministic screenshot manifest entries for all camera presets', () => {
    ...
    expect(manifest[0]?.fileName.endsWith('.png')).toBe(true);
    expect(manifest.some((row) => row.cameraDistance === 'near')).toBe(true);
    expect(manifest.some((row) => row.cameraDistance === 'mid')).toBe(true);
    expect(manifest.some((row) => row.cameraDistance === 'far')).toBe(true);
  });
});
