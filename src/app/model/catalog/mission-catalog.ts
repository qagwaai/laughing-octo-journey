...
export const MISSION_CATALOG: readonly MissionDefinition[] = [
  ...
  },
  {
    ...
    objectives: ['Mine Magnesium from the asteroid field.', 'Survive the hostile scavenger encounter.'],
...
export function resolveNewlyUnlockedMissionIds(
  ...
  return MISSION_CATALOG.filter(
    (m) =>
      ...
      m.prerequisites.every((prereqId) => prereqId === justCompletedMissionId || alreadyCompletedIds.has(prereqId)),
  ).map((m) => m.id);
}
