...
describe('drive-profile model', () => {
  ...
  it('returns quantum fold drive for high-tier ships', () => {
    expect(resolveDriveProfileForShip({ model: 'Carrier Prime', tier: 6 } as any)).toEqual(QUANTUM_FOLD_DRIVE_PROFILE);
  });

  it('returns quantum fold drive when model implies quantum engine', () => {
    expect(resolveDriveProfileForShip({ model: 'Quantum Scout', tier: 2 } as any)).toEqual(QUANTUM_FOLD_DRIVE_PROFILE);
  });
