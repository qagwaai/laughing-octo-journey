...
describe('ship-list correlation integration', () => {
  ...
  it('routes concurrent ship-list responses to the correct wrappers', fakeAsync(() => {
    ...
    const shipExteriorCallback = vi.fn();
    ...
    expect(shipServiceRequest.correlationId).toEqual(expect.any(String));
    expect(shipExteriorRequest.correlationId).toEqual(expect.any(String));
  ...
  it('dispatches socket-correlation-warning when dropping mismatched ship-list response', fakeAsync(() => {
    ...
    try {
      ...
      const customEvent = warningSpy.mock.calls.at(-1)?.[0] as CustomEvent;
