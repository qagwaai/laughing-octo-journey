...
describe('OpeningAudioService', () => {
  class MockAudioParam {
    ...
    exponentialRampToValueAtTime = jasmine.createSpy('exponentialRampToValueAtTime').and.callFake((value: number) => {
      this.value = value;
    });
  }
  ...
  beforeEach(() => {
    ...
    setIntervalSpy = spyOn(window, 'setInterval').and.callFake(((handler: TimerHandler) => {
  ...
  it('should stop the cinematic bed and clear scheduled layers', async () => {
    ...
    const createdOscillators = context.createOscillator.calls
      .all()
      .map((call) => call.returnValue as unknown as MockOscillatorNode);
