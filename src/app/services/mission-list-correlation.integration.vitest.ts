...
describe('mission-list correlation integration', () => {
  ...
  it('routes concurrent mission-list responses to the correct service wrappers', async () => {
    ...
    const missionBoardCallback = vi.fn();
    ...
    expect(requestFromMissionService.correlationId).toEqual(expect.any(String));
    expect(requestFromMissionBoard.correlationId).toEqual(expect.any(String));
  ...
  it('keeps strict mission-list matching when responses omit correlation metadata', async () => {
    let missionServiceResult: ListMissionsResult | undefined;
    const missionServicePromise = missionService.listMissions({
      playerName: 'Pioneer',
      characterId: 'char-a',
