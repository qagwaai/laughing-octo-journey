...
describe('MissionService connectivity integration', () => {
  ...
  it('returns not-connected when connect event does not arrive before timeout', async () => {
    vi.useFakeTimers();
    try {
      const pending = missionService.listMissions({
        playerName: 'Pioneer',
        characterId: 'char-1',
  ...
  it('proceeds after delayed connect and resolves mission-list response', async () => {
    const pending = missionService.listMissions({
      playerName: 'Pioneer',
      characterId: 'char-1',
      sessionKey: 'session-1',
