...
describe('socket-correlation helpers', () => {
  it('creates operation-prefixed correlation ids', () => {
    ...
    expect(correlationId.startsWith('mission-list:')).toBeTrue();
  ...
  it('matches request identities after normalization', () => {
    ...
    expect(matchesBasicRequestIdentity(left, right)).toBeTrue();
  });

  it('does not match request identities when one side is missing', () => {
    expect(matchesBasicRequestIdentity(undefined, { operation: 'mission-list' })).toBeFalse();
    expect(matchesBasicRequestIdentity({ operation: 'mission-list' }, undefined)).toBeFalse();
  });
