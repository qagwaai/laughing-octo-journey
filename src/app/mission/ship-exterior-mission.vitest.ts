...
describe('evaluateMissionGateOnScan', () => {
  ...
  it('does not change gate state for a non-iron scan sample', () => {
    ...
    const copperSample = {
      ...makeIronScanSample(),
      revealedMaterial: { rarity: 'Common' as const, material: 'Copper', textureColor: '#b86c45' },
    };
...
describe('hasMissionGatePendingRetry', () => {
  it('returns true when a step is pending-retry', () => {
    ...
    expect(hasMissionGatePendingRetry(state)).toBe(true);
  });

  it('returns false when no step is pending-retry', () => {
    expect(hasMissionGatePendingRetry(makeInitialState())).toBe(false);
  });
});
...
describe('parseMissionGateState — additional edge cases', () => {
  ...
  it('returns null when characterId does not match', () => {
    ...
    expect(
      parseMissionGateState({ rawStatusDetail: raw, missionId: 'first-target', characterId: 'c-1', steps }),
    ).toBeNull();
  });
