...
describe('MissionScenePlugin', () => {
  ...
  it('default seed policy delegates to the mission definition', () => {
    ...
    const definition = {
      missionId: 'unit-test',
      createFallbackAsteroidSamples: vi.fn().mockReturnValue(fallback),
      createNewAsteroidSamplesAroundShip: vi.fn().mockReturnValue(fallback),
      createResumedAsteroidSamples: vi.fn().mockReturnValue(fallback),
    } as never;
  ...
  it('applies hud and hook overrides', () => {
    ...
    expect(plugin.hudConfig.showRepairBay).toBe(false);
    expect(plugin.hudConfig.showFabricationLab).toBe(true);
  ...
  it('resolveMissionScenePlugin uses a registered factory when present', () => {
    ...
    expect(plugin.hudConfig.showScanPanel).toBe(false);
