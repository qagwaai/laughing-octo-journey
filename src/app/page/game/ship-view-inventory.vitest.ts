...
function setup(options: {
  ...
  const mockSocketLifecycle = {
    runWhenConnected: vi.fn().mockImplementation((callback: () => void) => callback()),
  };
  const mockConsumedItemShadowService = {
    filterInventory: vi
      .fn()
      .mockImplementation((_playerName: string, _characterId: string, inventory: any[]) => inventory ?? []),
  };
...
describe('ShipViewInventoryPage', () => {
  ...
  it('should split groups by tier for the same item type', () => {
    ...
    const groups = component['inventoryGroups']() as InventoryGroup[];
    expect(groups.length).toBe(2);
    expect(groups.every((group) => group.itemType === 'sensor-array')).toBe(true);
  ...
  it('should mark the highest-tier grouped component as equipped', () => {
    ...
    expect(equippedBadges.every((badge) => badge.textContent?.trim() === 'E')).toBe(true);
    expect(equippedBadges.every((badge) => badge.getAttribute('title') === 'Equipped tier')).toBe(true);
  });
  ...
  it('should toggle active sort class between item and tier headers', () => {
    ...
    expect(itemSortButton?.classList.contains('active-sort')).toBe(true);
    expect(tierSortButton?.classList.contains('active-sort')).toBe(false);

    component['toggleSort']('tier');
    fixture.detectChanges();

    expect(itemSortButton?.classList.contains('active-sort')).toBe(false);
    expect(tierSortButton?.classList.contains('active-sort')).toBe(true);
  });
  ...
  it('does not call upsertItem when ship context or session is missing', () => {
    ...
    (socketService as unknown as { upsertItem: ReturnType<typeof vi.fn> }).upsertItem = upsertSpy;
  ...
  it('adds drone item into ship inventory on successful upsert', () => {
    ...
    (socketService as unknown as { upsertItem: ReturnType<typeof vi.fn> }).upsertItem = vi
      .fn()
      .mockImplementation((_request: any, cb: (response: any) => void) => {
        cb({
          success: true,
          item: {
            id: 'drone-1',
    ...
    expect(component['joinShip']()?.inventory?.some((item: any) => item.id === 'drone-1')).toBe(true);
  });
  ...
  it('adds sensor-array item with prompted tier on successful upsert', () => {
    ...
    (socketService as unknown as { upsertItem: ReturnType<typeof vi.fn> }).upsertItem = vi
      .fn()
      .mockImplementation((request: any, cb: (response: any) => void) => {
  ...
  it('clamps prompted sensor-array tier to 1..20 before upsert', () => {
    ...
    vi.spyOn(window, 'prompt').mockReturnValue('999');
    (socketService as unknown as { upsertItem: ReturnType<typeof vi.fn> }).upsertItem = vi
      .fn()
      .mockImplementation((request: any, cb: (response: any) => void) => {
  ...
  it('routes add-dart-drone dev action through the dispatcher', () => {
    ...
    const addDroneSpy = vi.spyOn(component as any, 'addDroneToInventory').mockImplementation(() => undefined);
  ...
  it('routes add-sensor-array dev action through the dispatcher', () => {
    ...
    const addSensorSpy = vi.spyOn(component as any, 'addSensorArrayToInventory').mockImplementation(() => undefined);
  ...
  it('does not request ship refresh when required context is missing', () => {
    ...
    mockShipService.listShipsByOwner.mockClear();
  ...
  it('repairs empty scavenger starter inventory via item-upsert and re-fetches ship data', () => {
    ...
    const upsertItemSpy = vi.fn().mockImplementation((request: any, cb: (response: any) => void) => {
      cb({
        success: true,
        message: 'ok',
        playerName: 'Pioneer',
    ...
    (socketService as unknown as { upsertItem: ReturnType<typeof vi.fn> }).upsertItem = upsertItemSpy;
