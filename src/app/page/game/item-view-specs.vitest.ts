...
describe('ITEM_VIEW_SPECS_CONFIGS', () => {
  it('contains a config for Scavenger Pod', () => {
    expect(ITEM_VIEW_SPECS_CONFIGS.has('Scavenger Pod')).toBe(true);
  });

  it('contains a config for Expendable Dart Ship', () => {
    expect(ITEM_VIEW_SPECS_CONFIGS.has('Expendable Dart Ship')).toBe(true);
  });

  it('contains a config for expendable-dart-drone', () => {
    expect(ITEM_VIEW_SPECS_CONFIGS.has('expendable-dart-drone')).toBe(true);
  });

  it('contains a config for basic-mining-laser', () => {
    expect(ITEM_VIEW_SPECS_CONFIGS.has('basic-mining-laser')).toBe(true);
  });

  it('contains a config for structural-frames', () => {
    expect(ITEM_VIEW_SPECS_CONFIGS.has('structural-frames')).toBe(true);
  });

  it('contains a config for basic-plating', () => {
    expect(ITEM_VIEW_SPECS_CONFIGS.has('basic-plating')).toBe(true);
  });
...
describe('ItemViewSpecsPage', () => {
  ...
  it('should set imageNotFound to true on image error', () => {
    ...
    expect(component['imageNotFound']()).toBe(false);
    ...
    if (candidates.length > 1) {
      ...
      expect(component['imageNotFound']()).toBe(true);
    }
  });
...
describe('ItemViewSpecsPage - overlay close button', () => {
