...
interface FacadeHarness {
  ...
  services: {
    solarSystemService: { getSolarSystem: ReturnType<typeof vi.fn> };
    marketService: { listMarketsByLocation: ReturnType<typeof vi.fn> };
    shipService: { listShipsByOwner: ReturnType<typeof vi.fn> };
    socketService: { upsertShip: ReturnType<typeof vi.fn> };
  };
}
...
describe('ViewerDataFacade', () => {
  ...
  it('hydrates market stations when none exist and drops stale responses', () => {
    ...
    services.marketService.listMarketsByLocation.mockImplementation((request: any, cb: (response: any) => void) => {
      marketCallback = cb;
  ...
  it('maps market names and merges market stations into current bodies', () => {
    ...
    let marketCallback: ((response: any) => void) | null = null;
    services.marketService.listMarketsByLocation.mockImplementation((_request: any, cb: (response: any) => void) => {
      marketCallback = cb;
    });
  ...
  it('filters ships by current system and keeps ships without spatial', () => {
    ...
    services.shipService.listShipsByOwner.mockImplementation((_request: unknown, cb: (response: any) => void) => {
      cb({
        success: true,
        ships: [
  ...
  it('repairs invalid ship spatial once and reloads ships on successful upsert', () => {
    ...
    services.socketService.upsertShip.mockImplementation((_request: unknown, cb: (response: any) => void) => {
