...
describe('MarketHubPage', () => {
  ...
  it('should use response docking state to enable transact only at docked market', () => {
    ...
    const sorted = component['localMarkets']();
    expect(component['canTransactAtMarket'](sorted[0])).toBe(true);
    expect(component['canTransactAtMarket'](sorted[1])).toBe(false);
  });
  ...
  describe('helper formatting and edge branches', () => {
    ...
    it('returns unknown distance required-drive label for invalid in-system distance', () => {
      ...
      expect(component['isMarketWithinDriveRange'](market)).toBe(false);
    });
    ...
    it('allows transact when response marks market itself as docked', () => {
      ...
      expect(component['canTransactAtMarket'](market)).toBe(true);
    });
