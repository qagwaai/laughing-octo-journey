...
export default class PrintQueuePage {
  ...
  protected activeShip = signal<ShipSummary | null>(
    this.navigationState.joinShip ?? this.sessionService.activeShip() ?? null,
  );
  ...
  protected cancelPrintJob(item: PrintQueueItem): void {
    ...
    this.restorePrintableMaterials(
      ...
      (restoredItems) => {
        ...
        this.collectingItemIds.delete(item.id);
        this.printerSuccess.set(
          `Print job canceled. ${this.describeConsumedMaterials(consumedMaterials)} returned to ship inventory.`,
        );
      },
    );
  }
