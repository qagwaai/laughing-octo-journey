...
type MockBustAdapter = {
  readCharacterBust: ReturnType<typeof vi.fn>;
  createCharacterBust: ReturnType<typeof vi.fn>;
  updateCharacterBust: ReturnType<typeof vi.fn>;
};
...
function createMockBustAdapter(): MockBustAdapter {
  return {
    readCharacterBust: vi.fn().mockImplementation((request: { characterId: string }) =>
      of({
        success: true,
        message: 'bust read',
    ...
    createCharacterBust: vi.fn().mockImplementation((request: { characterId: string }) =>
      of({
        success: true,
    ...
    updateCharacterBust: vi.fn().mockImplementation((request: { characterId: string }) =>
      of({
        success: true,
...
describe('CharacterSetupPage', () => {
  ...
  it('should preload existing bust descriptor values in edit mode', async () => {
    ...
    bustAdapter.readCharacterBust.mockReturnValue(
      of({
        success: true,
        message: 'bust read',
        correlationId: 'bust-corr-read',
  ...
  it('should auto-persist bust changes in edit mode after selector changes', async () => {
    ...
    await fixture.whenStable();
    bustAdapter.updateCharacterBust.mockClear();
  ...
  describe('saveCharacter()', () => {
    ...
    it('should navigate after create even if bust save does not settle', async () => {
      const bustAdapter = createMockBustAdapter();
      bustAdapter.createCharacterBust.mockReturnValue(NEVER);
    ...
    it('should keep user on setup page when edit bust save returns blocked-save response', async () => {
      const bustAdapter = createMockBustAdapter();
      bustAdapter.updateCharacterBust.mockReturnValue(
        of({
          success: false,
      ...
      expect(component['bustBlockedSave']()).toEqual(
        expect.objectContaining({
          blockedSave: expect.objectContaining({ reason: 'DATABASE_ERROR', retryable: true }),
        }),
      );
  ...
  describe('createStarterShipForCharacter() — cold-boot item provisioning', () => {
    ...
    it('should emit owner-scoped ship list after successful character-add', async () => {
    ...
    it('should set warningMessage when ship-list-response fails', async () => {
      const { component, fixture } = setup({ socketService, sessionService });
      triggerSuccessfulCharacterAdd(component, 'c-1');
      await fixture.whenStable();
    ...
    it('should set warningMessage when ship-list-response has no ships', async () => {
      const { component, fixture } = setup({ socketService, sessionService });
      triggerSuccessfulCharacterAdd(component, 'c-1');
      await fixture.whenStable();
    ...
    it('should emit ship-upsert-request when ship-list-response succeeds with a ship', async () => {
      const { component, fixture } = setup({ socketService, sessionService });
      triggerSuccessfulCharacterAdd(component, 'c-1');
      await fixture.whenStable();
    ...
    it('should navigate before starter ship provisioning settles', async () => {
      const { component, mockRouter, fixture } = setup({ socketService, sessionService });
      triggerSuccessfulCharacterAdd(component, 'c-1');
      await fixture.whenStable();
    ...
    it('should clear warningMessage after successful ship upsert', async () => {
      const { component, fixture } = setup({ socketService, sessionService });
      ...
      expect(socketService.emittedEvents.some((e) => e.event === 'item-upsert-request')).toBe(false);
    });
