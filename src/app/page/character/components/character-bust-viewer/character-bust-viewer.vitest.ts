...
describe('CharacterBustViewerComponent', () => {
  ...
  it('resolves a distinct render profile per selector state', () => {
    ...
    expect(focused).toEqual(expect.objectContaining({ faceShape: 'oval', hairStyle: 'short-crop' }));
    expect(warm).toEqual(expect.objectContaining({ expressionPreset: 'warm', apparelAccent: 'visor' }));
  ...
  it('resolves deterministic camera poses', () => {
    expect(resolveBustCameraPose('three-quarter')).toEqual(
      expect.objectContaining({ position: [2.15, 1.3, 4.35], target: [0, 1.12, 0] }),
    );
    expect(resolveBustCameraPose('front')).toEqual(
      expect.objectContaining({ position: [0, 1.24, 5.15], target: [0, 1.08, 0] }),
    );
  });
  ...
  it('applies preset changes and captures camera state from orbit controls', () => {
    ...
    const controls = {
      ...
      reset: vi.fn(),
      saveState: vi.fn(),
      getAzimuthalAngle: vi.fn().mockReturnValue(0.42),
      getPolarAngle: vi.fn().mockReturnValue(1.1),
    };
    ...
    expect(component['viewerReady']()).toBe(true);
  ...
  it('resets orientation to the default preset', () => {
    ...
    const controls = {
      ...
      reset: vi.fn(),
      saveState: vi.fn(),
      getAzimuthalAngle: vi.fn().mockReturnValue(1.4),
      getPolarAngle: vi.fn().mockReturnValue(1.7),
    };
