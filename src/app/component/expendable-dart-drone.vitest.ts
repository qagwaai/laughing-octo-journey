...
import { BEFORE_RENDER_FN, ExpendableDartShip, GLTF_RESOURCE_FN } from './expendable-dart-drone';
...
describe('ExpendableDartShip', () => {
  ...
  let gltfResourceSpy: ReturnType<typeof vi.fn>;
  ...
  beforeEach(async () => {
    ...
    beforeRenderSpy = vi.fn().mockImplementation((callback: any) => {
      beforeRenderCallbacks.push(callback);
      return () => {};
    });

    gltfResourceSpy = vi.fn().mockReturnValue({
      asReadonly: vi.fn().mockReturnValue({
  ...
  describe('Model loading', () => {
    ...
    it('should return null if model is not loaded', () => {
      const nullGltfSpy = vi.fn().mockReturnValue({
        asReadonly: vi.fn().mockReturnValue({
    ...
    it('should call gltfResource with correct model path', () => {
      expect(gltfResourceSpy).toHaveBeenCalledWith(expect.any(Function), expect.any(Object));
    });
  });
  ...
  describe('beforeRender hook', () => {
    ...
    it('should rotate mesh on beforeRender', () => {
      ...
      (component as any).meshRef = vi.fn().mockReturnValue({
    ...
    it('should handle missing mesh reference gracefully', () => {
      (component as any).meshRef = vi.fn().mockReturnValue(null);
  ...
  describe('Template and attributes', () => {
    ...
    it('should include NgtArgs in imports', () => {
      ...
      const hasNgtArgs =
        metadata.dependencies && Array.isArray(metadata.dependencies)
