...
describe('Asteroid', () => {
  ...
  let injectStoreSpy: ReturnType<typeof vi.fn>;
  ...
  beforeEach(async () => {
    ...
    injectStoreSpy = vi.fn().mockReturnValue({
      snapshot: {
        camera: {
  ...
  it('should remove scan rings after completion when morph pulse has finished and asteroid is not hovered', () => {
    ...
    expect((component as any).showScanFx()).toBe(false);
  ...
  it('should show material dialog only when scanned and hovered with material', () => {
    ...
    expect((component as any).showResultDialog()).toBe(false);

    (component as any).emitHover(true);
    expect((component as any).showResultDialog()).toBe(true);
  ...
  it('should emit right-button pointer down events for button or bitmask input', () => {
    ...
    expect(downSpy.mock.calls.at(-1)?.[0]).toEqual({ id: 'sample-1', button: 2 });
  });
