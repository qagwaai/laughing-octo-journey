...
import { BEFORE_RENDER_FN, Earth, TEXTURE_RESOURCE_FN } from './earth';

describe('Earth', () => {
  let component: Earth;
  let fixture: ComponentFixture<Earth>;
  let beforeRenderSpy: ReturnType<typeof vi.fn>;
  let textureResourceSpy: ReturnType<typeof vi.fn>;
  ...
  beforeEach(async () => {
    ...
    textureResourceSpy = vi.fn().mockReturnValue({
      asReadonly: vi.fn().mockReturnValue({
        value: vi.fn().mockReturnValue(mockTextures),
      }),
    });
  ...
  describe('Texture loading', () => {
    ...
    it('should return null if textures are not loaded', () => {
      const nullTextureSpy = vi.fn().mockReturnValue({
        asReadonly: vi.fn().mockReturnValue({
    ...
    it('should call textureResource with correct texture paths', () => {
      ...
      const callArgs = textureResourceSpy.mock.calls[0];
