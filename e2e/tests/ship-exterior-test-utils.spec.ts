...
test.describe('Ship Exterior Test Utilities', () => {
	test('supports deterministic mission progression without real 3D pointer raycast', async ({ page }) => {
		...
		expect(gateAfterForcedScan!.steps.find((step) => step.key === 'neutralize_identified_asteroid')?.status).toBe('active');
		...
		expect(manufactureWithoutLaunch.steps.find((step) => step.key === 'manufacture_hull_patch_kit')?.status).not.toBe('completed');
	...
	test('completes deterministic mission flow and emits completed mission status', async ({ page }) => {
		...
		await expect
			.poll(async () =>
				page.evaluate(() => {
					const api = (window as Window & {
						__shipExteriorTestUtils?: { getMissionGateState: () => { steps: Array<{ key: string; status: string }> } };
					}).__shipExteriorTestUtils;
					const gate = api!.getMissionGateState();
					return {
	...
	test('resets stale local completed gate state when backend reports started without statusDetail', async ({ page }) => {
		const mock = new SocketIOMock(page);
		await mock.setup();
		...
		expect(gateAfterBackendRefresh.steps.find((step) => step.key === 'manufacture_hull_patch_kit')?.status).toBe('locked');
	...
	test('reopens final step when backend reports in-progress without statusDetail and local gate is fully completed', async ({ page }) => {
		const mock = new SocketIOMock(page);
		await mock.setup();
		...
		expect(gateAfterBackendRefresh.steps.find((step) => step.key === 'identify_iron_asteroid')?.status).toBe('completed');
		expect(gateAfterBackendRefresh.steps.find((step) => step.key === 'manufacture_hull_patch_kit')?.status).toBe('completed');
